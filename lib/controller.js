...
exports.addPresentation = function(res, presId, url, password) {
  var error = {};
    ...
    presentations.findById(presId)
      .then(function(presentation) {
        if (presentation) {
          ...
          savePresentation(res, presId, url, password);
        }
      });
  } else {
    // User did not choose a presId. Save with a random one.
    createNewPresId().then(function(newPresId){
      savePresentation(res, newPresId, url, password);
    });
  }
};

function savePresentation(res, presId, url, password){
  presentations.save(presId, url, password).then(
    function(result){
      if (!result) {
        res.status(404);
      } else {
        res.redirect("/presentations/" + presId + "/publicize");
      }
    });
}
 ...
 * PR用ページ表示
 */
exports.showPublicizePage = function(res, presId, presentationUrl) {
  presentations.findById(presId)
    .then(function(presentation) {
      if (presentation) {
        res.render('publicize', {presentationUrl: presentationUrl});
      } else {
...
/**
 * @return PR用ページのURLを返す(QRCodeGenerator:http://goqr.me/api/)
 */
function makePublicizePageUrl(urlBase, presId){
  return urlBase + presId + "/publicize";
